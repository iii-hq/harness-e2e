...
function metric(
  ...
  return {
    ...
    delta,
    relativeDelta:
      delta === null || baseline === null || baseline === 0
        ? null
        : delta / Math.abs(baseline),
  }
}
...
export function ReleaseControlPlanDetailPage({ planKey }: { planKey: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (next.mode !== 'local')
      throw new Error('Release Control references require the local dashboard')
    setBridge(next)
    ...
    const executions = remote
      .filter((execution) => execution.release_control?.profile === planKey)
      .sort((left, right) =>
    ...
    const related = local.plans.filter(
      (plan) =>
  ...
  const comparisonMetrics = Object.fromEntries(
    comparison.metrics
      ...
      .map((entry) => [
        entry.id === 'function_calls'
          ? 'functionCalls'
          : entry.id === 'function_errors'
            ? 'functionErrors'
            : entry.id,
        metric(entry.baseline, entry.candidate),
      ]),
  ) as Partial<
    Record<
      | 'score'
  ...
  const planName = reference
    ? text(reference.execution.plan.name) || planKey
    : planKey
      ...
      <div className="ds-root page-shell w-[calc(100%_-_1.5rem)] max-w-[1420px] pt-5 pb-16 md:w-[calc(100%_-_3rem)]">
        ...
        {loading ? (
          ...
          <>
            ...
            <ExecutionComparisonPanel
              headingId="plan-reference-comparison"
              title="Reference and local candidate"
              summary="Reference: Release Control · Candidate: Local"
              metrics={comparisonMetrics}
              showDeltas={Boolean(candidateResult)}
              actions={
                candidateId ? (
                  <a
                    className={buttonClassName({ variant: 'quiet' })}
            ...
            >
              ...
              selectedLocalPlan.reference_execution_id !==
                ...
                <Callout tone="info">
                  ...
                  {selectedLocalPlan.reference_execution_id}. You are comparing
                  it with {reference?.execution.id}.
                </Callout>
              ) : null}
