...
const fieldLabel =
  'grid min-w-0 gap-2 text-xs font-semibold text-[var(--ds-color-ink-soft)]'
const fieldControl =
  'min-h-11 w-full rounded-lg border border-[var(--ds-color-line)] bg-[var(--ds-color-surface-raised)] px-3 text-sm text-[var(--ds-color-ink)] transition-colors duration-[var(--ds-duration-fast)] placeholder:text-[var(--ds-color-ink-muted)] hover:border-[var(--ds-color-line-strong)] focus-visible:border-[var(--ds-color-focus)] focus-visible:[outline:2px_solid_var(--ds-color-focus)] focus-visible:[outline-offset:3px] disabled:cursor-not-allowed disabled:opacity-50'
const quietButton =
  'inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-[var(--ds-color-line)] bg-[var(--ds-color-surface-raised)] px-3 text-xs font-semibold text-[var(--ds-color-ink-soft)] transition-colors duration-[var(--ds-duration-fast)] hover:border-[var(--ds-color-line-strong)] hover:text-[var(--ds-color-ink)] disabled:cursor-not-allowed disabled:opacity-45'
...
function SetupSection({
  ...
  return (
    <section className="border-b border-[var(--ds-color-line)] p-5 last:border-b-0 sm:p-6">
      <div className="grid grid-cols-[2rem_minmax(0,1fr)] items-start gap-3">
        <span className="pt-0.5 font-mono text-[0.68rem] font-semibold text-[var(--ds-color-brand-text)]">
          {number}
        </span>
        <div className="min-w-0">
          <h2 className="m-0 text-sm font-semibold tracking-[-0.015em] text-[var(--ds-color-ink)]">
            {title}
          </h2>
...
export function ExecutionSetup({
  ...
  return (
    <div className="min-w-0 bg-[var(--ds-color-surface)]">
      <div
        className="flex flex-wrap items-center gap-3 border-b border-[var(--ds-color-line)] bg-[var(--ds-color-surface-raised)] px-5 py-3 sm:px-6"
        aria-live="polite"
      >
        <span
          className={`h-2 w-2 rounded-full ${availableScenarios.length > 0 ? 'bg-[var(--ds-color-success)]' : 'bg-[var(--ds-color-warning)]'}`}
          aria-hidden="true"
        />
        <span className="min-w-0 flex-1 text-xs text-[var(--ds-color-ink-soft)]">
          {catalogSummary}
        </span>
        <code className="max-w-full truncate font-mono text-[0.68rem] text-[var(--ds-color-ink-muted)] sm:max-w-[16rem]">
          {url || 'URL not loaded'}
        </code>
        {onRefreshCatalog && (
          <button
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className={fieldLabel} htmlFor={`${idPrefix}-label`}>
            <span className="flex items-baseline justify-between gap-2">
              ...
              <small className="font-normal text-[var(--ds-color-ink-muted)]">
      ...
      >
        <div className="grid gap-4">
          <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end">
            ...
            >
              ...
              <span className="relative">
                <Search
                  className="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-[var(--ds-color-ink-muted)]"
                  size={15}
                  aria-hidden="true"
                />
                <input
          ...
          <output className="flex min-h-10 flex-wrap items-center justify-between gap-2 border-y border-[var(--ds-color-line)] py-2 font-mono text-[0.68rem] text-[var(--ds-color-ink-muted)]">
            <span>
              <strong className="text-[var(--ds-color-ink)]">
          ...
          <div className="grid max-h-[25rem] grid-cols-1 gap-2 overflow-auto rounded-lg border border-[var(--ds-color-line)] bg-[var(--ds-color-surface-raised)] p-2 sm:grid-cols-2">
              ...
              return (
                <label
                  className={`flex min-h-16 min-w-0 cursor-pointer items-start gap-3 rounded-lg border p-3 transition-colors duration-[var(--ds-duration-fast)] ${
                    selected
                      ? 'border-[color-mix(in_srgb,var(--ds-color-brand)_60%,var(--ds-color-line))] bg-[color-mix(in_srgb,var(--ds-color-brand)_7%,var(--ds-color-surface))]'
                      : 'border-[var(--ds-color-line)] bg-[var(--ds-color-surface)] hover:border-[var(--ds-color-line-strong)]'
                  }`}
                  key={scenario}
                >
                  <input
                    className="mt-0.5 h-4 w-4 shrink-0 accent-[var(--ds-color-brand)]"
                    type="checkbox"
                    checked={selected}
                    disabled={disabled}
                    onChange={(event) =>
                  ...
                  <span className="grid min-w-0 gap-1">
                    <strong className="text-xs font-semibold leading-4 text-[var(--ds-color-ink)]">
                      {scenarioDisplayName(scenario)}
                    </strong>
                    <code className="break-all font-mono text-[0.62rem] leading-4 text-[var(--ds-color-ink-muted)]">
                      {scenario}
                    </code>
                  </span>
                </label>
            ...
            {visibleScenarios.length === 0 && (
              <p className="col-span-full m-0 p-6 text-center text-xs text-[var(--ds-color-ink-muted)]">
      ...
      <details className="group bg-[color-mix(in_srgb,var(--ds-color-surface-raised)_60%,transparent)]">
        <summary className="flex min-h-20 cursor-pointer list-none items-center justify-between gap-4 px-5 py-4 marker:content-none sm:px-6 [&::-webkit-details-marker]:hidden">
          <span className="grid min-w-0 gap-1">
            <span className="font-mono text-[0.65rem] font-semibold uppercase tracking-[0.06em] text-[var(--ds-color-brand-text)]">
              Optional controls
            </span>
            <strong className="text-sm font-semibold text-[var(--ds-color-ink)]">
              Sampling, retries and seed
            </strong>
            <small className="text-xs leading-4 text-[var(--ds-color-ink-muted)]">
              Retries recover technical failures and never add evidence samples.
            </small>
          </span>
          <span className="shrink-0 font-mono text-xs text-[var(--ds-color-ink-soft)]">
            {runsPerScenario} / test ·{' '}
            {Math.max(0, Number(technicalRetries) || 0)} retries
          </span>
        </summary>
        <div className="grid gap-px border-t border-[var(--ds-color-line)] bg-[var(--ds-color-line)] sm:grid-cols-3">
          <label className={`${fieldLabel} bg-[var(--ds-color-surface)] p-5`}>
            Runs per test
            <span className="text-[0.68rem] font-normal leading-4 text-[var(--ds-color-ink-muted)]">
...
export function ExecutionSetupReview({
  ...
  return (
    <aside className="grid content-start bg-[var(--ds-color-surface-raised)] lg:sticky lg:top-20 lg:max-h-[calc(100dvh-6rem)] lg:overflow-auto">
      <div className="border-b border-[var(--ds-color-line)] p-5 sm:p-6">
        ...
        <div className="mt-3 flex items-start justify-between gap-3">
          <div>
            <h2 className="m-0 text-base font-semibold text-[var(--ds-color-ink)]">
              Review setup
            </h2>
            <p className="mt-1 mb-0 text-xs leading-5 text-[var(--ds-color-ink-muted)]">
              {mode === 'plan'
          ...
          <span
            className={`inline-flex min-h-7 shrink-0 items-center rounded-full border px-2.5 font-mono text-[0.62rem] font-semibold ${
              ready
                ? 'border-[color-mix(in_srgb,var(--ds-color-success)_35%,transparent)] text-[var(--ds-color-success)]'
                : 'border-[var(--ds-color-line-strong)] text-[var(--ds-color-ink-muted)]'
            }`}
          >
            {status}
          </span>
      ...
      <dl className="grid grid-cols-2 gap-px bg-[var(--ds-color-line)]">
        {facts.map((fact) => (
          <div
            className="grid min-h-24 content-between gap-3 bg-[var(--ds-color-surface)] p-4"
            key={fact.label}
          >
            <dt className="font-mono text-[0.62rem] uppercase tracking-[0.05em] text-[var(--ds-color-ink-muted)]">
              {fact.label}
            </dt>
            <dd className="m-0 font-mono text-xl font-semibold tracking-[-0.04em] text-[var(--ds-color-ink)]">
              {fact.value}
            </dd>
          </div>
        ))}
      ...
      <dl className="grid gap-4 border-t border-[var(--ds-color-line)] p-5 text-xs sm:p-6">
        <div className="grid gap-1">
          <dt className="font-mono text-[0.62rem] uppercase tracking-[0.05em] text-[var(--ds-color-ink-muted)]">
            Execution model
          </dt>
          <dd className="m-0 break-all text-[var(--ds-color-ink)]">
        ...
        <div className="grid gap-1">
          ...
          <dd className="m-0 break-all text-[var(--ds-color-ink-soft)]">
            {judge || 'Automatic'}
          </dd>
        </div>
        <div className="grid gap-1">
          ...
          <dd className="m-0 break-all font-mono text-[0.68rem] text-[var(--ds-color-ink-soft)]">
            {url || 'Not loaded'}
          </dd>
        </div>
      </dl>

      <div className="grid gap-3 border-t border-[var(--ds-color-line)] p-5 sm:p-6">
        {children}
      </div>
    </aside>
  )
