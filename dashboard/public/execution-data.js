(function initHarnessExecutionData(root, factory) {
  ...
  function compareExecutions(leftValue, rightValue) {
    ...
    const leftStackMode = ["source", "registry"].includes(left.stack?.mode)
      ? left.stack.mode
      : null;
    const rightStackMode = ["source", "registry"].includes(right.stack?.mode)
      ? right.stack.mode
      : null;
    const stackCompatible =
      leftStackMode === null ||
      rightStackMode === null ||
      leftStackMode === rightStackMode;
    ...
    const scenarios = keys.map((key) => {
      ...
      const comparable = stackCompatible && contract === "same";
      return {
        key,
    ...
    if (leftStackMode === null || rightStackMode === null) {
      warnings.push("Stack mode is unavailable; compatibility is not verifiable.");
    } else if (!stackCompatible) {
      warnings.push(
        `Stack modes are incompatible (${leftStackMode} versus ${rightStackMode}).`,
      );
    }
    const sameSubjects = stableJson(leftSubjects) === stableJson(rightSubjects);
    const sameRequestedRuns =
    ...
    const contractsCompatible = missingCount === 0 && changedCount === 0;
    const comparable =
      leftStackMode !== null &&
      rightStackMode !== null &&
      stackCompatible &&
    ...
    const totals = Object.fromEntries(
      totalFields.map((field) => {
        ...
        return [field, {
          left: leftValue,
          right: rightValue,
          delta: comparable ? metricDelta(leftValue, rightValue) : null,
        }];
      }),
    );
    return {
      ...
      comparable,
      compatibility:
        leftStackMode === null || rightStackMode === null
          ? "legacy_unverified"
          : comparable
            ? "eligible"
            : "ineligible",
