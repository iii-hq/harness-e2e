...
- O banco do E2E não tem número de versão nem passo de migração: cada tabela guarda o fingerprint das instruções que a criam e, ao iniciar, o worker recria as tabelas cujo fingerprint mudou em uma transação, mantendo execuções, planos e receipts que o binário atual ainda lê e reprojetando runs a partir dos bundles nativos. Evidência ausente é declarada sem inventar score; resultados ou planos escritos sob outro contrato são lidos com aviso, nunca recusados.
- Linhas ilegíveis, tabelas de layouts anteriores e o histórico importado do Release Control numa tabela recriada são descartados com aviso; o histórico volta por nova importação. Planos escritos por outro binário são apagados na leitura seguinte, nunca migrados.
