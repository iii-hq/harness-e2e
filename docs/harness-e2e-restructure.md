...
- O banco do E2E não tem número de versão: o worker grava um fingerprint do próprio layout e do contrato de resultados e recusa um banco gravado sob outro fingerprint. `rebuild-storage` faz dry-run por padrão e, com `--apply`, recria todas as tabelas em uma transação mantendo execuções, planos e receipts que o binário atual ainda lê. Execuções ativas impedem a reconstrução; evidência ausente é declarada sem inventar score.
