...
- O banco do E2E não tem número de versão: o worker grava um fingerprint do próprio layout SQL e recusa um banco gravado sob outro fingerprint; resultados ou planos escritos sob outro contrato são lidos com aviso, nunca recusados. `rebuild-storage` faz dry-run por padrão e, com `--apply`, recria todas as tabelas em uma transação mantendo execuções, planos e receipts que o binário atual ainda lê. Execuções ativas impedem a reconstrução; evidência ausente é declarada sem inventar score.
- A reconstrução descarta linhas ilegíveis, tabelas de layouts anteriores e o histórico importado do Release Control, que volta por nova importação. Planos escritos por outro binário são apagados na leitura seguinte, nunca migrados.
