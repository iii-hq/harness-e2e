...
2. **Preparação:** implementar e verificar migrações em cópias isoladas. Preservar IDs, hashes e arquivos originais; produzir relatório de dados interpretáveis, lacunas e conflitos. A evolução local do storage acontece no início do worker, tabela por tabela, com aviso do que foi recriado.
...
| Persistência Harness | Sem versão de storage: fingerprint por tabela gravado pelo worker, que recria ao iniciar as tabelas cujo layout mudou; planos e receipts compostos em SQL pelo database worker, verificados por hash |
