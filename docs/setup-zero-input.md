...
A retirada dessas entradas do setup deve vir acompanhada da identidade efetiva obtida do artefato instalado ou contrato da stack. Manter versão/digest de runtime e fixture na evidência. O launcher exact-stack já deriva parte dessas informações de contratos; reutilizar essa resolução sem inventar identidade quando ela não estiver disponível. [Geração da campanha](../scripts/exact_stack_campaign.py), [preparação da execução](../scripts/prepare_execution.py).
