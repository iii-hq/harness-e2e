...
Name the API Compose service `api` and the frontend service `web` and provide Node and Playwright Chromium there so the executor can inspect the real page. With the service's configured working directory and environment, `docker compose -f <compose_file> exec -T web node -e "require('@playwright/test')"` must succeed. The executor launches Chromium in that service and uses the internal port mapped to `WEB_PORT`. Keep runtime version and checksum evidence in your report.
