...
""" + results_block
    ...
    def test_template_router_keeps_configuration_and_private_provider_credentials(self):
        contract = campaign_contract({
            "harness": "1.9.0", "state": "0.22.1", "llm-router": "1.4.20",
            "provider-deepseek": "0.1.12",
        })
        contract["orchestration"]["roots"].append(
            {"worker": "provider-deepseek", "version": "0.1.12", "role": "runtime"}
        )
        template = {"containers": {"llm-router": {
            "worker": "package://llm-router", "version": "1.0.0",
            "env_file": ["./.env"], "start_after": ["state"],
        }}}
        project = MODULE.project_scaffold(contract, "project-one", Path("/tmp/runs"),
            {"provider-deepseek": "/private/provider.env"}, {}, template)
        router = project["containers"]["llm-router"]
        self.assertEqual(router["version"], "1.4.20")
        self.assertEqual(router["env_file"], ["/private/provider.env"])
        self.assertEqual(router["start_after"], ["state"])
        self.assertEqual(template["containers"]["llm-router"]["env_file"], ["./.env"])
        self.assertIn("provider-deepseek", project["containers"]["harness"]["start_after"])
