...
use super::catalog::{
    CapturedWorkflowAsset, StepCatalog, StepEvaluation, StepExecutorContext, StepExecutorOutput,
    TypedPortValue, WorkflowAssetContent, WorkflowCleanupContext, WorkflowEvaluationResult,
    WorkflowGateResult,
};
use super::{
...
pub async fn execute_workflow(
    ...
    for node_id in &materialized.topological_order {
        let node = &nodes[node_id];
        let registered = catalog
            .get(&node.step_type, node.step_version)
            .expect("validated catalog entry");
        if let Err(error) = registered.executor.cleanup_workflow(&cleanup_context).await {
            technical_failure = true;
            let report = reports.get_mut(node_id).expect("known step report");
            report
                .failures
