...
fn capture<'a>(
    ...
    Box::pin(async move {
        ...
        let invariants = vec![
            CapturedInvariant {
                id: "forward_correct".to_string(),
                passed: verification.forward_correct(),
                reason: "column, backfill, and index checked by the runner".to_string(),
            },
            CapturedInvariant {
                id: "rollback_restores".to_string(),
                passed: verification.rollback_restores(),
                reason: "schema and rows compared against the seeded database".to_string(),
            },
        ];
        Ok(vec![kit::evidence(
            DELIVERABLE_ID,
