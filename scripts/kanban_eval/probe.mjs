...
async function main() {
  ...
  const { check, stage } = createCheckRecorder(checks, async (id) => {
    ...
    // Failed flows must not leave intercepted requests or SSE clients in later checks.
...
export function boardTicketTotal(page, count) {
  ...
  const text = page.getByText(new RegExp(`^(?:${count}\\s+tickets?(?:\\s+on\\s+(?:the\\s+)?board\\.?)?|total(?:\\s+tickets?)?\\s*:?\\s*${count}(?:\\s+tickets?)?)$`, 'i'))
...
export function boardEmptyFeedback(page) {
  return page.getByText(/^(?:no tickets\b.*|(?:the )?board is empty[.!]?)$/i)
...
export function ticketCard(page, ticket) {
  const name = new RegExp(literalPattern(ticket.title))
...
export async function holdResponse(page, pattern, method) {
  ...
  const request = new Promise((resolve) => { reached = resolve })
  const response = new Promise((resolve) => { delivered = resolve })
  let intercepted = false
  await page.route(pattern, async (route) => {
    if (route.request().method() !== method || intercepted) { await route.continue(); return }
    intercepted = true
    try {
      ...
      delivered({})
    } catch (error) { reached({ error }); delivered({ error }) }
  })
  const bounded = async (signal, label) => {
    let timer
    try {
      const result = await Promise.race([signal, new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`${method} ${label} timed out`)), 8_000)
      })])
      if (result.error) throw result.error
    } finally { clearTimeout(timer) }
  }
  return { release, reached: () => bounded(request, 'request'), delivered: () => bounded(response, 'response') }
}

export async function assertTicketDetails(page, ticket) {
  const detail = await ticketDetail(page, ticket)
  const values = [
    [ticket.description, false], [ticket.status === 'todo' ? 'To do' : ticket.status.replaceAll('_', ' '), true],
    [ticket.priority, true], [ticket.assignee, false],
  ]
  for (const [value, presentation] of values) {
    if (value == null || value === '') continue
    const text = presentation ? new RegExp(`^${literalPattern(value)}$`, 'i') : value
    expect(await detail.getByText(text, { exact: true }).filter({ visible: true }).count() > 0,
      `detail omitted persisted value: ${value}`)
  }
  return detail
}

export function boardNavigation(page) {
  return page.getByRole('link', { name: /^(?:back to )?(?:the )?board[.!]?$/i }).filter({ visible: true })
}
...
export const PROBES = {
  ...
  async kanban_c3_board({ api, trigger, browser, baseUrl, output, check }) {
    ...
    // Each state starts in its own browser context and controls its own read.
    for (const state of ['loading', 'empty', 'error', 'retry']) {
      await check(`board_${state}`, async () => {
        const context = await browser.newContext()
        const page = await context.newPage()
        let fail = state === 'error' || state === 'retry'
        let release
        const held = new Promise((resolve) => { release = resolve })
        await context.route('**/api/tickets', async (route) => {
          ...
          await route.fulfill({ status: fail ? 500 : 200, contentType: 'application/json',
            body: JSON.stringify(fail ? { error: 'probe failure' } : { tickets: [] }) }).catch(() => {})
        })
        try {
          await page.goto(baseUrl)
          if (state === 'loading') {
    ...
    let seeded
    await check('board_fixture', async () => {
      const tickets = []
      for (const [status, label] of statuses) tickets.push(await create(trigger, { title: `${label} probe`, status, priority: 'high', assignee: 'Grace' }))
      seeded = tickets
    })
    await check('board_total', async () => {
      requireEvidence(seeded, 'Board fixture creation failed')
      const { context, page } = await pageFor(browser, baseUrl)
      await eventually(async () => await boardTicketTotal(page, seeded.length).count() === 1, 'board total is wrong')
      await context.close()
    })
    await check('board_columns', async () => {
      requireEvidence(seeded, 'Board fixture creation failed')
      const { context, page } = await pageFor(browser, baseUrl)
    ...
    await check('board_metadata', async () => {
      requireEvidence(seeded, 'Board fixture creation failed')
      const { context, page } = await pageFor(browser, baseUrl)
      for (let index = 0; index < statuses.length; index++) {
        const [, label] = statuses[index]
        const lane = boardLane(page, label)
        await eventually(async () => await lane.getByText(seeded[index].key, { exact: true }).count() === 1, `${label} readable key is missing`)
    ...
    await check('board_refresh', async () => {
      const { context, page } = await pageFor(browser, baseUrl)
      await boardRefreshButton(page).waitFor({ state: 'visible' })
      let ticketReads = 0
      ...
      await eventually(async () => await boardTicketTitle(page, refreshed.title).count() > 0, 'refresh did not load the new ticket')
      await context.close()
    })
    await check('board_settings', async () => {
    ...
    await check('board_safe_text', async () => {
      const markup = '<img src=x onerror=alert(1)> literal title'
      await create(trigger, { title: markup })
      const { context, page } = await pageFor(browser, baseUrl)
      await eventually(async () => await boardTicketTitle(page, markup).count() > 0, 'literal ticket title did not load')
      expect(await page.locator('img[src="x"]').count() === 0, 'ticket markup created an image element')
      await context.close()
    })
    await check('board_responsive', async () => {
      const ticket = await create(trigger, { title: 'Responsive board probe' })
      const { context, page } = await pageFor(browser, baseUrl)
      await eventually(async () => await boardTicketTitle(page, ticket.title).count() > 0, 'responsive fixture did not load')
      for (const [name, width, height] of [['desktop', 1280, 900], ['mobile', 390, 844]]) {
        await page.setViewportSize({ width, height })
        expect(!await page.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth), `${name} page has document-wide horizontal overflow`)
        await screenshot(page, output, `board-${name}`)
      }
      await context.close()
    })
  },
  ...
  async kanban_c4_ticket_flow({ api, trigger, control, browser, baseUrl, output, check }) {
    const fields = { description: 'UTF-8: ação e café', status: 'in_review', priority: 'urgent', assignee: 'Lin' }
    const newForm = async (title) => {
      const opened = await pageFor(browser, baseUrl, { width: 390, height: 844 })
      await opened.page.getByRole('button', { name: 'New ticket' }).click()
      const dialog = opened.page.getByRole('dialog', { name: 'New ticket' })
      await dialog.getByLabel('Title').fill(title)
      await dialog.getByLabel('Description').fill(fields.description)
      await dialog.getByLabel('Status').selectOption(fields.status)
      await dialog.getByLabel('Priority').selectOption(fields.priority)
      await dialog.getByLabel('Assignee').fill(fields.assignee)
      return { ...opened, dialog }
    }
    await check('ticket_cards', async () => {
      const ticket = await create(trigger, { title: 'Keyboard card probe' })
      const { context, page } = await pageFor(browser, baseUrl)
      const boardUrl = page.url()
      const card = ticketCard(page, ticket)
      await card.focus()
      await card.press('Enter')
      await ticketDetail(page, ticket)
      expect(context.pages().length === 1, 'keyboard card opened another tab')
      await page.goBack()
      await eventually(() => page.url() === boardUrl, 'history did not return to the board')
      await card.click()
      await ticketDetail(page, ticket)
      expect(context.pages().length === 1, 'pointer card opened another tab')
      await context.close()
    })
    await check('ticket_modal', async () => {
      const { context, page } = await pageFor(browser, baseUrl, { width: 390, height: 844 })
    ...
    await check('ticket_create_error', async () => {
      const { context, page, dialog } = await newForm('Failed creation draft')
      await page.route('**/api/tickets', (route) => route.request().method() === 'POST'
      ...
      expect(await dialog.isVisible(), 'failed create closed the modal')
      for (const [label, value] of [['Title', 'Failed creation draft'], ['Description', fields.description], ['Status', fields.status], ['Priority', fields.priority], ['Assignee', fields.assignee]]) {
        expect(await dialog.getByLabel(label).inputValue() === value, `failed create erased ${label}`)
      }
      await page.unroute('**/api/tickets')
      await dialog.getByRole('button', { name: 'Create ticket' }).click()
      await eventually(async () => !await dialog.isVisible(), 'creation retry did not close the modal')
      const ticket = (await json(await api('/api/tickets'))).tickets.find(({ title }) => title === 'Failed creation draft')
      expect(ticket, 'creation retry did not persist its draft')
      await context.close()
    })
    await check('ticket_creation', async () => {
      const title = 'Browser-created ticket'
      const { context, page, dialog } = await newForm(title)
      const [response] = await Promise.all([
        page.waitForResponse((response) => new URL(response.url()).pathname === '/api/tickets' && response.request().method() === 'POST'),
        dialog.getByRole('button', { name: 'Create ticket' }).click(),
      ])
      expect(response.status() === 201, 'successful creation did not return HTTP 201')
      const { ticket } = await response.json()
      expect(ticket?.title === title, 'creation returned the wrong ticket')
      const persisted = (await json(await api(`/api/tickets/${ticket.id}`))).ticket
      for (const [field, value] of Object.entries({ title, ...fields })) {
        expect(ticket[field] === value && persisted[field] === value,
          `browser creation did not persist submitted ${field}`)
      }
      const detail = await ticketDetail(page, ticket)
      expect(!await dialog.isVisible(), 'create dialog stayed open')
      expect(await detail.locator('xpath=ancestor-or-self::*[self::dialog or @role="dialog" or @aria-modal="true"]').count() === 0, 'ticket details opened in a modal')
      expect(context.pages().length === 1, 'ticket creation opened another tab')
      await context.close()
    })
    await check('ticket_identifiers', async () => {
      const ticket = await create(trigger, { title: 'Identifier probe' })
      expect((await json(await api(`/api/tickets/${ticket.id}`))).ticket.key === ticket.key, 'HTTP UUID lookup failed')
      expect((await json(await api(`/api/tickets/${ticket.key}`))).ticket.id === ticket.id, 'HTTP key lookup failed')
      expect((await trigger('kanban::tickets::get', { id: ticket.id })).key === ticket.key, 'iii UUID lookup failed')
      expect((await trigger('kanban::tickets::get', { id: ticket.key })).id === ticket.id, 'iii key lookup failed')
    })
    ...
    await check('ticket_mobile', async () => {
      const ticket = await create(trigger, { title: 'Mobile detail probe', ...fields })
      const { context, page } = await pageFor(browser, baseUrl, { width: 390, height: 844 })
      await ticketCard(page, ticket).click()
      await ticketDetail(page, ticket)
      const link = page.url()
      ...
      await direct.goto(link)
      await ticketDetail(direct, ticket)
      expect(!await direct.evaluate(() => document.documentElement.scrollWidth > document.documentElement.clientWidth), 'mobile detail has document-wide overflow')
      await screenshot(direct, output, 'ticket-detail-mobile')
      await context.close()
    })
    await check('ticket_delete_error', async () => {
      ...
      await page.route('**/api/tickets/**', (route) => route.request().method() === 'DELETE'
        ? route.fulfill({ status: 500, contentType: 'application/json', body: '{"error":"probe failure"}' }) : route.continue())
      await detail.getByRole('button', { name: 'Delete ticket' }).click()
      await expectText(mutationFailureFeedback(detail), /\S/)
      await ticketDetail(page, ticket)
      expect((await json(await api(`/api/tickets/${ticket.id}`))).ticket.id === ticket.id, 'failed delete removed the persisted ticket')
      await context.close()
    })
    await check('ticket_delete_navigation', async () => {
      ...
      try {
        ...
        await boardNavigation(page).first().click()
        delayed.release()
        await delayed.delivered()
        await eventually(async () => !(await json(await api('/api/tickets'))).tickets.some(({ id }) => id === ticket.id), 'deleted card remained listed')
        await eventually(async () => await ticketCard(page, remaining).count() === 1, 'board did not load after navigating during deletion')
        await eventually(async () => await ticketCard(page, ticket).count() === 0, 'deleted card remained visible after the late response')
      } finally { delayed.release() }
      await context.close()
    })
    await check('ticket_durable_deletion', async () => {
      const ticket = await create(trigger, { title: 'Durable deletion probe' })
      const response = await api(`/api/tickets/${ticket.key}`, { method: 'DELETE' })
      expect(response.status === 200, 'HTTP deletion by key failed')
      expect((await json(response)).ticket.id === ticket.id, 'deletion returned the wrong ticket')
      expect((await api(`/api/tickets/${ticket.id}`)).status === 404, 'deleted ticket is still retrievable')
      let deleted = JSON.parse(await control('read_store')).find(({ id }) => id === ticket.id)
      expect(deleted?.deleted_at, 'soft-deleted ticket is absent from disk')
      await control('restart')
      deleted = JSON.parse(await control('read_store')).find(({ id }) => id === ticket.id)
      ...
      expect(Number(next.key.slice(4)) > Number(ticket.key.slice(4)), 'deleted readable key was reused')
    })
    await check('ticket_content_type', async () => {
      expect([400, 415].includes((await api('/api/tickets', { method: 'POST', body: 'not json' })).status), 'non-JSON creation was not rejected')
    })
    await check('ticket_utf8', async () => {
      ...
      const chunked = await json(await api('/api/tickets', { method: 'POST', headers: { 'content-type': 'application/json' }, body, duplex: 'half' }))
...
export function createCheckRecorder(checks, onFailure = async () => {}, onComplete = async () => {}) {
  let active
