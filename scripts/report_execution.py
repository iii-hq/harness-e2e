...
def identity_of(args: argparse.Namespace, artifacts: Path | None) -> dict[str, Any]:
    ...
    # The workflow is read from the default branch while the executor is pinned
    # per campaign, so what it passes has to be what every pinned revision
    # understands: the resolution summary, by that name.
    ...
    return prune(
        {
            ...
            # A worker built from a commit reports its Cargo version; the
            # commit is what says which code ran.
            "stack_commits": resolution.get("stack_commits"),
