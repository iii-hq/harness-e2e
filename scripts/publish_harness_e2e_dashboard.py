...
def _public_failures(value: Any) -> list[dict[str, Any]]:
    if not isinstance(value, list):
        return []
    return [
        _pick(failure, ("domain", "phase", "message"))
...
def _public_deliverables(value: Any) -> list[dict[str, Any]]:
    ...
    for item in value:
        ...
        deliverable["invariants"] = _public_json(item.get("invariants", []))
        deliverable["provenance"] = _public_json(item.get("provenance", []))
        deliverable["preview"] = _public_json(item.get("preview"))
        if isinstance(item.get("artifact"), dict):
            deliverable["artifact"] = _pick(
                item["artifact"],
                ("id", "kind", "path", "sha256", "size_bytes", "media_type"),
            )
    ...
    return [
        {
            **_pick(item, ("dimension", "passed")),
            "signals": _public_json(item.get("signals")),
        }
        for item in value
...
def _public_scenario(value: Any) -> dict[str, Any] | None:
    ...
    if isinstance(value.get("case"), dict):
        scenario["case"] = _public_json(value["case"])
    aggregate = _pick(
        value.get("aggregate"),
        (
            "runs",
